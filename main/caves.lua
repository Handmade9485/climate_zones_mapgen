...
function M.gen_caves(gen_data)
	...
	nobj_surface_caves = nobj_surface_caves or core.get_value_noise_map(config.caves_surface_np, dims)
	nobj_deep_caves = nobj_deep_caves or core.get_value_noise_map(config.caves_depths_np, dims)
	...
	for y = 0, gen_data.sidelen-1 do
		...
		local y_i = 1 + y * gen_data.sidelen
		for z = 0, gen_data.sidelen-1 do
			local yz_i = y_i + z * gen_data.sidelen2
